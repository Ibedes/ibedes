        ...
        const bellUnreadBadge = document.querySelector(
            "[data-unread-badge]",
        );
        const notificationTrigger = document.querySelector(
            "[data-notification-trigger]",
        );
        const resetFeedButton = document.querySelector(
            "[data-reset-feed]",
        );
        ...
        const baseNotifications = Array.isArray(
            parsedNotifications,
        )
            ? parsedNotifications
            : [];
        ...
        const initialNotifications = Array.isArray(
            storedNotifications,
        )
            ? storedNotifications
            : baseNotifications;
        ...
        let notificationState = initialNotifications;
        const defaultNotifications = [...baseNotifications];
        ...
        const updatePriorityCount = (items) => {};
        ...
        const refreshNotifications = (override) => {
        ...
        const setNotificationState = (
            ...
            persistNotificationState(notificationState);
        ...
        const renderNotifications = (items) => {
            ...
            items.forEach((item) => {
                ...
                iconWrapper.className = "admin-notification-card__icon-wrapper";
                ...
                headerRight.className = "admin-notification-card__header-right";
                
                const typeBadge = document.createElement("span");
                typeBadge.className = "admin-notification-card__badge";
                typeBadge.dataset.type = item.type;
                typeBadge.textContent = formatNotificationTypeLabel(item.type);
                
                const time = document.createElement("span");
                time.className = "admin-notification-card__time";
                ...
                // Body with title and message
                const body = document.createElement("div");
                body.className = "admin-notification-card__body";
                
                const title = document.createElement("h3");
                title.className = "admin-notification-card__title";
                title.textContent = item.title;
                
                const message = document.createElement("p");
                message.className = "admin-notification-card__message";
                ...
                footer.className = "admin-notification-card__footer";
                
                const statusBadge = document.createElement("div");
                statusBadge.className = "admin-notification-card__status";
                statusBadge.dataset.status = item.status ?? "unread";
                
                const statusIcon = document.createElement("i");
                statusIcon.className = (item.status ?? "unread") === "unread" 
                    ? "fa-solid fa-circle-dot" 
                    : "fa-solid fa-circle-check";
                
                const statusText = document.createElement("span");
                statusText.textContent = (item.status ?? "unread") === "unread" 
                    ? "Baru" 
                    : "Dibaca";
                ...
                if (item.href) {
                    ...
                    link.className = "admin-notification-card__action";
                    link.href = item.href;
                    link.innerHTML = 'Lihat <i class="fa-solid fa-arrow-right"></i>';
        ...
        const resetFeed = () => {
            setNotificationState([...defaultNotifications]);
        };

        const normalizeCommentNotification = (record) => {
        ...
        const normalizeLikeNotification = (record) => {
            if (!record) return null;
            const slug = String(record.slug ?? "").replace(/^\/+|\/+$/g, "");
            return {
                ...
                (payload: any) => {
                    ...
                    if (!exists) {
                        setNotificationState(
                            [normalized, ...notificationState].slice(0, 15),
                        );
                    }
                },
            );
        ...
        const bootstrapNotifications = async () => {
            ...
            try {
                ...
                if (items.length) {
                    setNotificationState(items.slice(0, 15));
                }
            } catch (error) {
...
<style>
    ...
    .admin-notification {
        ...
        min-height: 100%;
        box-shadow: 
            0 1px 3px color-mix(in srgb, var(--color-foreground) 5%, transparent),
            0 1px 2px color-mix(in srgb, var(--color-foreground) 3%, transparent);
    ...
    .admin-notification:hover {
        ...
        box-shadow: 
            ...
            0 2px 4px -1px color-mix(in srgb, var(--color-foreground) 4%, transparent);
        transform: translateY(-1px);
    }

    .admin-notification.is-new {
    ...
    .admin-notification__icon[data-type="like"] {
        background: color-mix(in srgb, #ff6b9a 12%, transparent);
        color: #ff6b9a;
    }
    ...
    .admin-notification__icon[data-type="system"] {
        background: color-mix(in srgb, var(--color-warning, #f6c744) 15%, transparent);
    ...
    .admin-notification__meta-bar {
        ...
        border-radius: 0 0 calc(var(--admin-radius) - 0.25rem)
            calc(var(--admin-radius) - 0.25rem);
    }
    ...
    .admin-pill-chip[data-priority-chip="high"] {
        background: color-mix(in srgb, #ff6b6b 16%, transparent);
        border-color: color-mix(in srgb, #ff6b6b 30%, transparent);
        color: color-mix(in srgb, #ff6b6b 90%, transparent);
    }

    .admin-pill-chip[data-priority-chip="low"] {
        background: color-mix(in srgb, var(--color-foreground) 8%, transparent);
    }
    ...
    .admin-notification-card {
        ...
        background: var(--color-card, var(--color-background));
        box-shadow: 
            0 1px 3px color-mix(in srgb, var(--color-foreground) 6%, transparent),
            0 1px 2px color-mix(in srgb, var(--color-foreground) 4%, transparent);
    ...
    .admin-notification-card:hover {
        ...
        box-shadow: 
            0 10px 15px -3px color-mix(in srgb, var(--color-foreground) 8%, transparent),
            0 4px 6px -2px color-mix(in srgb, var(--color-foreground) 4%, transparent);
        transform: translateY(-2px);
    }

    .admin-notification-card[data-status="unread"] {
        border-left: 4px solid var(--color-primary);
        background: color-mix(in srgb, var(--color-primary) 3%, var(--color-card, var(--color-background)));
    }
    ...
    .admin-notification-card__icon-wrapper {
        flex-shrink: 0;
    }
    ...
    .admin-notification-card__icon[data-type="like"] {
        background: color-mix(in srgb, #ff6b9a 12%, transparent);
        color: #ff6b9a;
    }

    .admin-notification-card__icon[data-type="comment"] {
        background: color-mix(in srgb, #4f46e5 12%, transparent);
        color: #4f46e5;
    }

    .admin-notification-card__icon[data-type="message"] {
        background: color-mix(in srgb, var(--color-foreground) 10%, transparent);
        color: color-mix(in srgb, var(--color-foreground) 70%, transparent);
    }
    ...
    .admin-notification-card__badge[data-type="like"] {
        background: color-mix(in srgb, #ff6b9a 15%, transparent);
        color: #ff6b9a;
    }

    .admin-notification-card__badge[data-type="comment"] {
    ...
    .admin-notification-card__title {
        font-size: 1.125rem;
        font-weight: 700;
        line-height: 1.4;
        margin: 0;
    ...
    .admin-notification-card__status[data-status="unread"] i {
        color: var(--color-primary);
        animation: pulse-dot 2s ease-in-out infinite;
    }

    .admin-notification-card__status i {
        font-size: 0.625rem;
    }
    ...
    .admin-notification-card__action:hover {
        background: color-mix(in srgb, var(--color-primary) 15%, transparent);
        border-color: color-mix(in srgb, var(--color-primary) 30%, transparent);
        transform: translateX(2px);
    }

    .admin-notification-card__action i {
        font-size: 0.75rem;
        transition: transform 200ms ease;
    }

    .admin-notification-card__action:hover i {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        const fetchOverviewData = async () => {
            try {
                ...
                updateText("metric-total-views", data.pageViews.toLocaleString());
        ...
        const updateMetric = (id, value) => {
            ...
            if (valueEl) valueEl.textContent = typeof value === 'number' ? value.toLocaleString() : value;
                ...
                newValueEl.textContent = typeof value === 'number' ? value.toLocaleString() : value;
